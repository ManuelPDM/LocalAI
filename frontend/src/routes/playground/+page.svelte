<!-- File: frontend/src/routes/playground/+page.svelte (Modified) -->
<script>
    // NOTE: This is the complete script. The main change is removing the global `toggleDarkMode` function.
    ...
    onMount(async () => {
        ...
        // NOTE: The dark mode check is now handled by the global layout, but we keep this
        // for safety in case this page is ever rendered without it.
        if (typeof document !== 'undefined' && localStorage.getItem('theme') === 'dark') {
            document.body.classList.add('dark-mode');
        }

        autoPlayEnabled = (typeof localStorage !== 'undefined') && localStorage.getItem(AUTO_PLAY_KEY) === 'true';
    ...
    const playTextAsSpeech = async (text, buttonElement) => {
        ...
        try {
            ...
            currentAudio.onended = () => {
                isAudioPlaying = false;
                currentAudio = null;
                if (typeof document !== 'undefined' && document.body.contains(buttonElement)) {
            ...
            if (typeof document !== 'undefined' && document.body.contains(buttonElement) && !isAudioPlaying) {
                buttonElement.classList.remove('playing');
                buttonElement.disabled = false;
            }
        }
...
<div class="playground-wrapper">
    ...
    <div class="sidebar" class:open={showMobileMenu}>
        ...
        <div class="sidebar-footer">
            ...
            <div class="footer-action">
                ...
                <input type="checkbox" id="voice-chat-toggle" title="Toggle automatic voice playback for new messages" bind:checked={autoPlayEnabled} on:change={() => { if(typeof localStorage !== 'undefined') localStorage.setItem(AUTO_PLAY_KEY, autoPlayEnabled) }}>
            </div>
            <!-- REMOVED global settings and dark mode from here -->
            <div class="footer-action">
                <label for="settings-btn">⚙️ Session Settings</label>
    ...
    <div id="full-voice-overlay" class:hidden={!isFullVoiceModeActive} on:click={stopVoiceLoop} role="button" tabindex="0" on:keydown={(e) => {if(e.key==='Enter') stopVoiceLoop()}}></div>
...
<style>
    /* This style block is identical to your previous one, scoped to this component */
    .playground-wrapper {
        display: flex;
        height: 100%;
        width: 100%;
